...
/**加载动画*/
/**加载动画显示*/
+ (void)loadAnimation:(RootViewController *)vc {
    //TODO: 加载动画修改
    if (vc.loadImageView) return;
    vc.view.userInteractionEnabled = NO;
    UIImageView *imageView = [[UIImageView alloc] init];
    imageView.center = vc.view.center;
    imageView.bounds = CGRectMake(0, 0, 80, 80);
    vc.loadImageView = imageView;
    [vc.view addSubview:imageView];
//    [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.size.mas_equalTo(CGSizeMake(80, 80));
//        make.center.equalTo(vc.cyl_tabBarController.view);
//    }];
    ...
    for (int i = 1; i <= 12; i++) {
        // 计算文件名
        NSString *filename = [NSString stringWithFormat:@"jiazaidonghua-%d.png", i];
        // 加载图片
    ...
    vc.loadImageView.animationImages = images;
    
    // 2.设置播放次数(1次)
    vc.loadImageView.animationRepeatCount = 0;
    
    // 3.设置播放时间
    vc.loadImageView.animationDuration = images.count * 0.1;
    
    [vc.loadImageView startAnimating];
}

/**加载动画消失*/
+ (void)disappearAnimation:(RootViewController *)vc {
//    CGFloat delay = vc.loadImageView.animationDuration + 0.1;
//    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        // 4.动画放完后清除内存
        vc.view.userInteractionEnabled = YES;
        [vc.loadImageView stopAnimating];
        vc.loadImageView.hidden = YES;
        [vc.loadImageView removeFromSuperview];
        vc.loadImageView = nil;
//    });
}
...
- (void)screenShotWithView:(UIView *)view {
    //这里因为我需要全屏接图所以直接改了，宏定义iPadWithd为1024，kSCREEN_HEIGHT为768，
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(kSCREEN_WIDTH * mulriple, kSCREEN_HEIGHT * mulriple), YES, 0);     //设置截屏大小
    [[view layer] renderInContext:UIGraphicsGetCurrentContext()];
    UIImage *viewImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    CGImageRef imageRef = viewImage.CGImage;
    //    CGRect rect = CGRectMake(166, 211, 426, 320);//这里可以设置想要截图的区域
    CGRect rect = CGRectMake(0, 0, kSCREEN_WIDTH * mulriple, kSCREEN_HEIGHT * mulriple);//这里可以设置想要截图的区域
